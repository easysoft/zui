...
```js
import {Pick} from 'zui/picker';

const picker = new Pick(element, options);
```

### 通过全局对象 `zui`

```js
const picker = new zui.Pick(element, options);
```
...
import {Pick} from 'zui/picker/react';

render(element, <Pick {...options} />);
```
