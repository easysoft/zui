...
`@zui/event-bus` 基于浏览器原生 [`EventTarget`](https://developer.mozilla.org/zh-CN/docs/Web/API/EventTarget) 提供事件订阅与派发能力，包含三个层次的类：

* `EventEmitter`：最基础的事件发射器；
* `EventBus`：类型安全的事件总线，`emit` 字符串事件时自动创建 `Event` 或 `CustomEvent`；
* `EventHub`：支持自定义事件后缀，并提供 `offAll` 批量移除。
