    ...
    render(props: ProgressCircleOptions) {
        ...
        return (
            <svg className={className} width={size} height={size}>
                ...
                {text ? <text x={textX ?? center} y={textY ?? (center + (circleWidth / 2))} dominant-baseline="middle" text-anchor="middle" style={textStyle || {fontSize: `${radius}px`, stroke: 'currentColor'}}>{text === true ? Math.floor(percent) : text}</text> : null}
                {children}
            </svg>
        );
    }
