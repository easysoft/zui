...
export class CalendarEventDom<P extends CalendarEventProps = CalendarEventProps> extends HElement<P, {isExtend: boolean}> {
    ...
    render(): VNode<Attributes> {
        ...
        return (
            <div ref={this.calendarContentRef} class="calendar-event">
                {
                    calendarEvents?.map((event, index) => {
                    // 判断它的groupId是否存在于eventSetMap中，如果存在，则取出对应的颜色，否则取出默认颜色
                        return (
                            <div style={{backgroundColor: this.getColor(event)}} onClick={() => onEventClick && onEventClick(event)} data-event={event.id} data-index={index} data-date={new Date(event.date)} draggable={true} class="calendar-event-item" key={event.id}>
                                <span class="calendar-event-item-time">{event.date.getHours() + ':' + event.date.getMinutes()}</span>
                ...
                <div className="calendar-body-bottom">
                    {maxVisibleEvents && Number(prevLen) > maxVisibleEvents && (this.state.isExtend ? <span onClick={() => {this.setState({isExtend: false});}}><span class="chevron-up"></span></span> : <span onClick={() => this.setState({isExtend: true})}><span class="chevron-down"></span></span>)}
                </div>
            </div>
        );
    }
