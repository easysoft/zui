const defaultCols = [
    ...
    {name: 'manager', title: '负责人', width: 60, sortType: false, flex: 1, type: 'avatar', avatarKey: 'managerAvatar', avatarWithName: true, avatarClass: 'size-xs circle'},
    {name: 'progress', title: '进度', width: 65, align: 'center', sortType: false, type: 'circleProgress'},
    {name: 'storyPoints', title: '需求规模', width: 80, align: 'right', sortType: false, type: 'format', format: {type: 'html', format: (val) => `${Number(val).toFixed(1)} <small class="text-gray">SP</small>`}},
    {name: 'executionCounts', title: '执行数', width: 70, align: 'center', sortType: false, type: 'format', format: {type: 'html', format: '{0} <small class="text-dark">迭代</small>'}},
    {name: 'investedDays', title: '已投入', width: 70, align: 'center', sortType: false, type: 'format', format: {type: 'html', format: '{0} <small class="text-dark">人天</small>'}},
    {name: 'startDate', title: '开始日期', width: 90, align: 'center', sortType: false, type: 'format', format: {type: 'datetime', format: 'yyyy年MM月dd日'}},
    {name: 'finishDate', title: '完成日期', width: 90, align: 'center', sortType: false, type: 'format', format: {type: 'datetime', format: 'yyyy年MM月dd日'}},
    {name: 'actions', title: '操作', width: 120, sortType: false, fixed: 'right', onRenderCell(_result, _rowID, col, rowData) {
        return [{
...
function generateRowData(index) {
    ...
    return {
        ...
        managerAvatar: `../../../assets/avatar/avatar-${1 + (managerIndex % 10)}.png`,
        ...
        parent: `${['', '', '', id - 1, id - 2, id - 3, id - 3, id - 4, id - 1, id - 1][id % 10]}`,
    };
}
...
const optionsOverride = {
    'dtable-advanced': {
        ...
        cols: {
            ...
            actions: {type: 'actionButtons', width: 100, onRenderCell: null, actionBtnData: {start: {title: '开始', icon: 'play'}, close: {title: '关闭', icon: 'off'}, edit: {title: '编辑', icon: 'pencil'}}},
        },
    },
    'dtable-layout-size': {
        width: 'auto',
    ...
    'dtable-responsive': {
        ...
        responsive: true,
        cols: {
            project: {fixed: false, flex: 1, width: 'auto'},
            manager: false,
            storyPoints: false,
            executionCounts: false,
            startDate: false,
    ...
    'dtable-flex': {
        ...
        data: 5,
        cols: {
            project: {fixed: false, flex: 3, width: 'auto'},
            investedDays: {flex: 2, width: 'auto'},
    ...
    'dtable-cols-fixed': {
        ...
        cols: {
            ...
            investedDays: {flex: 2, width: 'auto', minWidth: 100},
            progress: {flex: 1, width: 'auto', minWidth: 100},
            manager: false,
            storyPoints: false,
            executionCounts: false,
            startDate: false,
    ...
    'dtable-sort-type': {
        ...
        cols: {
            id: {sortType: 'up'},
            project: {fixed: false, flex: 3, width: 'auto', sortType: false},
            investedDays: {flex: 2, width: 'auto', sortType: true},
            progress: {flex: 1, width: 'auto', sortType: true},
            manager: false,
            storyPoints: false,
            executionCounts: false,
            startDate: false,
    ...
    'dtable-render-cell': {
        cols: {
            actions: {
                onRenderCell(_result, _rowID, col, rowData) {
                    return [{
                        html: rowData[col.name].map(action => {
                            if (action === 'start') {
