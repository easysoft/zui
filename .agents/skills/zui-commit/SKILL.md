...
- 评审阶段保持只读。已有提交授权只允许后续按已确认范围暂存和提交，不包含修复代码、刷新生成物或推送。
...
4. **审查每组变更**：
   ...
   有 confirmed finding 或疑似敏感信息时先按严重度和文件/行号报告。只读模式将 commit message 标记为“修复或明确接受后可用”；只有剩余变化仍构成独立完整提交时，才另外给出不含问题部分的分组建议。提交模式停止写操作并询问用户是修复还是明确接受风险；不确定事项单列为风险，不冒充 finding。修复或候选范围变化后，重新收集上下文、审查并验证。
5. **执行验证**：
