...
当前 dev 管线优先读取已有 `dev.md`，不存在时回退到 `README.md`。盘点或评审时同时记录两者是否存在以及实际页面来源；默认维护用户指定的 `README.md`，不要无理由新增或迁移到 `dev.md`。

调试页用于覆盖开发场景，不是官网正式文档。不要把两套示例语法混用。

示例容器、Markdown 中的实例 DOM、可复制代码和 `dev.ts` 生成的界面都遵循 [布局与样式规范](component.md#布局与样式)。新增或修改相关类名时，读取并执行 [CSS utilities 核实流程](utilities.md)，确认目标消费方式支持所用类；调试页能显示不能代替此项核实。
...
- 新建或扩展场景时，按请求及组件能力覆盖基础、主要变体、相关状态和交互边界。
...
3. 涉及渲染、交互或 HMR 时，复用或启动 `pnpm dev`，访问目标页验证对应场景。
4. 对本次新增或改变的交互检查相关状态、键盘及边界，确认控制台无重复初始化或异常。
5. 按 [共享工作流](workflow.md) 和 `AGENTS.md` 管理服务；只清理本任务的临时验证服务，保留复用服务及用户要求的持续预览。
