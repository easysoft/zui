...
缺少公开的 Grid、响应式或任意值类，不意味着禁止相应 CSS 能力。组件 CSS 的 `@apply` 优先级、原生 CSS 适用情形和 `style/index.ts` 统一入口遵循 [布局与样式规范](component.md#布局与样式)。内部合法的 `@apply` 不受公开类清单限制，例如 `@apply -grid -grid-cols-2;`。不要仅为某个示例自动扩展公共 utilities 或修改 Tailwind 配置；跨库改动仍遵循已有任务范围。
