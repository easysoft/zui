...
- 用户另行明确要求修复时，先完成评审，再按变更所属的 ZUI 实施技能直接处理请求范围内的修复；不要让修复诉求改变本次评审范围。
...
2. 重点检查本次变更是否引入或暴露以下问题：
   ...
   - Web Component 变更按 [专用规范](../zui-standards/references/web-component.md) 核对公开入口、注册时机、渲染目标推导、配置缓存、属性反射及断开清理；
