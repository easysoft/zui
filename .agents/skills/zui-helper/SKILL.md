---
name: zui-helper
description: "在 ZUI 主仓库的 lib/* 中设计、实现或修复函数、类、store 和其他 helper；需求明确时完成必要设计后直接实施。"
---
...
新增工具或能力前执行[先查找，再复用](../zui-standards/references/workflow.md#复用已有能力)，按行为及符号名检索已有 helper、类型与基础设施；设计中说明所复用的 API 或必须新增的缺口。

## 实施计划

按共享工作流形成实施计划；以下仅展开本次相关决策：
