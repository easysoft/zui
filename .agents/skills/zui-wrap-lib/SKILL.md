...
在修改任何目标库文件前，输出一份可直接实施的计划并请求用户明确确认，然后停止。计划至少包含：
...
- 运行时验收场景、静态验证、仍存在的假设和明确标记的“已批准范围”。

本技能只设置这一次确认。完全落在该批准范围内的 component/helper/dev/doc 子流程不重复确认；若目标、公开 API、包角色、UMD 资源或文件边界发生变化，停止并重新规划。
...
根据范围补充类型检查、文档同步/构建和 `pnpm dev` 的 `/<name>/` 交互验证。持续进程完成后主动结束。
