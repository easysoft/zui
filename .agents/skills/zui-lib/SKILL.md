...
尚无适用批准时，先完成所有选中技能的发现与设计，但不要修改文件。输出一份拟实施集成计划，至少包含：
...
尚无适用批准时，请求用户对整份计划明确确认后再实施；批准状态及包含修订的授权回复遵循共享工作流。按 [zui 共享工作流](../zui-standards/references/workflow.md) 复用本任务已有且仍完整适用的明确批准，包括 wrap-lib 的协调计划，不重复确认。超出批准范围时只暂停受影响部分及其依赖写操作，提交增量计划并确认；继续范围内不受影响的工作。始终服从当前协作模式。
