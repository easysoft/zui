---
name: zui-component
description: "在指定 `lib/*` 中设计、实现或优化 ZUI 3 组件，覆盖纯 CSS、Preact、Preact + ComponentFromReact、原生 Component DOM 增强器及控制器 + Preact 子视图。Use when a user asks to add, refactor, extend, or fix a ZUI component and its public consumption modes; 需求理解与组件设计完成后必须先给出计划并等待明确确认，再修改代码。"
---
...
2. 完整读取根目录 `AGENTS.md`、兄弟技能 `../zui-standards/references/workflow.md` 和 `../zui-standards/references/component.md`。涉及新包或元数据时再读 `../zui-standards/references/library.md`；涉及文档、调试页或国际化时再读对应 standards 参考。
...
3. 按实际需要实现样式根类、`--<component>-*` 变量、带 `-` 前缀的 Tailwind `@apply`、语义标签、键盘、焦点和 ARIA。
4. 仅在批准范围内调用或遵循 `$zui-i18n`、`$zui-doc`、`$zui-dev`；不要借机修改其他领域。
5. 运行与风险匹配的 lint、单库构建和调试页检查；仓库没有通用单测时，以类型检查、演示交互和文档示例覆盖验收场景。
6. 汇报实现文件、公开 API、验证结果和未验证风险，不自动提交。
