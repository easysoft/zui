...
2. 按当前任务读取对应规范：
   ...
   - 组件、调试页和文档示例的布局、CSS 声明与样式入口：[布局与样式规范](references/component.md#布局与样式)
...
4. 本技能的规范查询阶段保持只读。用户已要求实施时，主流程完成查询后继续进入相应扩展版实施技能，完成范围内实现与验证；仅指出技能名称不构成实施任务交付。由其他技能调用时只提供约束，不增加独立确认门禁，也不由本查询阶段修改文件。
