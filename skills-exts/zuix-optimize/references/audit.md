...
只编辑扩展项目的文档源，不手工编辑宿主同步生成目录；验证写入遵循共享工作流的隔离与批准规则。
...
4. 按依赖和验收边界拆成可独立验证的相关批次，规模由本次证据和风险决定；
5. 每批深审后形成决策完整的计划并直接实施，完成验证后再进入下一批。

不要把尚未读源码的项目写成“预计修复”。

## 基线与验收

只读审计用 package 解析、源码、`git -C <GIT_ROOT> diff` 和不会写 cache/产物的检查记录基线，不运行会更新 build/dist/docs、宿主 cache、依赖或 lockfile 的命令。

实施任务按共享工作流的宿主验证隔离与批准规则选择执行位置，补充尚未被有效结果覆盖的相关验证：
...
实施中新增项先记录证据与范围；范围内更新计划后继续，真正超出用户请求或存在无法合理判断的重大歧义时仅澄清受影响部分。
