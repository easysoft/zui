...
- 不修改 `zuiRoot` 的源码、注册、缓存、依赖或锁文件；若用户明确把宿主改动也纳入提交，它属于另一个 Git 根，必须单独分析和提交。
...
4. **审查每组变更**：检查调试残留、明显逻辑错误、遗漏的异常/空值分支、复制错误、公共 API 注释、无关文件、生成物和敏感信息。按文件与行号报告问题；提交模式下由用户选择修复或明确忽略，修复后重新收集上下文。
