...
4. **审查每组变更**：按“与代码评审技能协作”审查当前候选快照，另外检查提交专属的调试残留、无关文件、生成物和敏感信息。有 confirmed finding 时先按严重度和文件/行号报告，不确定事项单列为风险；只读模式将有问题的 commit message 标记为“修复或明确接受后可用”。仅有提交授权不包含修复；提交模式下，本任务已有仍适用的修复授权时，在评审后完成范围内修复、重新评审和验证，再继续提交。没有修复授权或需要接受未解决问题时，仍由用户决定；修复后重新收集上下文。
