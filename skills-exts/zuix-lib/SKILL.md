...
先完成所有选中领域的发现与设计，但不修改文件。输出一份集成计划，至少包含：
...
- 明确标记的“已批准范围”，精确列出目标库、公开 API、允许修改的扩展项目领域和文件边界，并声明宿主只读。

请求用户对整份计划明确确认，然后停止。只出现这一个确认门禁；回答问题、调整细节或局部同意不算确认。

若本技能由 `../zuix-optimize/SKILL.md` 调用，且目标、API、文件边界、宿主影响和验收场景已完整包含在其已批准范围，可复用该确认；范围变化时返回 optimize 重新规划。始终服从当前协作模式。

## 编排实施

1. 确认且当前模式允许编辑后，重新运行 resolver 并检查 `gitRoot` 状态。解析结果、宿主注册或目标位置发生变化时停止核对。
2. 仅修改已批准的 `targetLibRoot`；新库则只在已批准的 `plannedTargetLibRoot` 创建文件。package、依赖安装、lockfile、源码 lint、类型检查和测试全部从 `extensionRoot` 处理；不得修改 `zuiRoot` 的源码、package、lockfile、注册、缓存或生成目录。
...
7. 一旦目标、公开 API、包角色、package 标识、宿主影响或文件边界需要变化，停止实施，更新集成计划并再次确认；不让子技能自行扩大范围。
...
3. 按批准范围从 `zuiRoot` 使用 `pnpm dev:exts -- --lib=buildIn,<extsName>`、`pnpm docs:build:exts` 或宿主当前等价脚本进行调试和文档验证。先确认命令实际存在；不发明筛选参数。持续进程完成后主动结束。
