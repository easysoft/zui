---
name: zuix-helper
description: "在独立 ZUI 扩展项目中设计、实现或修复函数、类、store 和其他 helper；需求明确时完成必要规划后直接实施。"
---
...
2. 仅修改 `targetLibRoot` 及任务所需的扩展项目文件。安装依赖、更新扩展 lockfile、lint、类型检查和测试都从 `extensionRoot` 执行；不修改宿主源码、依赖、lockfile 或注册；宿主生成物和缓存写入遵循共享工作流的验证隔离与批准规则。
...
6. 若任务范围包含正式文档或调试页，按需读取并遵循 `../zuix-doc/SKILL.md` 或 `../zuix-dev/SKILL.md`；在共享任务范围内直接实施。
