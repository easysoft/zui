...
async function mountCheckableTable(page: Page): Promise<void> {
    ...
    await page.evaluate(async ({data, mainPath, checkablePath}) => {
        document.documentElement.classList.remove('dark');
        document.body.className = '';
        document.body.innerHTML = '<main id="e2e-dtable-contract"><div id="e2e-dtable"></div></main>';

        const [{DTable}, {checkable}] = await Promise.all([
            import(mainPath) as Promise<{DTable: new (selector: string, options: Record<string, unknown>) => unknown}>,
            import(checkablePath) as Promise<{checkable: unknown}>,
        ]);
    ...
    }, {data: rows, mainPath: '/lib/dtable/src/main.ts', checkablePath: '/lib/dtable/src/plugins/checkable/index.tsx'});
...
test.describe('DTable checkable', () => {
    ...
    test('checks and unchecks every row from the header checkbox', async ({page}) => {
        ...
        expect(await readChecks(page)).toEqual({header: false, rows: [false, false, false]});

        await headerLabel(page).click();
        expect(await readChecks(page)).toEqual({header: true, rows: [true, true, true]});
    ...
    test('checks and unchecks a single row from its own checkbox', async ({page}) => {
        ...
        expect(await readChecks(page)).toEqual({header: false, rows: [false, true, false]});
