...
/// ZUI change begin
        drawLabel: function(segment, easeDecimal, labelPosMap)
        {
            ...
            if(placement === 'outside') {
                ...
                var textHeight = options.scaleFontSize - 2;
                var labelPos = Math.round((y * 0.8 + chartHeightHalf) / textHeight);
                var maxPos = Math.floor(this.chart.width / textHeight);
                if(labelPosMap[isRight ? labelPos : (-labelPos)]) {
                    if(labelPos > 0) labelPos--;
                    else labelPos++;
                }
                while(labelPosMap[isRight ? labelPos : (-labelPos)] && labelPos < maxPos) labelPos++;
                if(labelPosMap[labelPos]) return;
                y = labelPos * textHeight + options.scaleFontSize / 2;
