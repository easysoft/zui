...
const kuid = '5fa24ade36d6c';
const editor = {
    "id": ["comment", "lastComment"],
    "tools": "simpleTools"
};
const K = KindEditor;
...
$(function() {
    const diffButton =
        '<button type="button" class="btn btn-mini btn-icon btn-strip"><i class="icon icon-code icon-sm"></i></button>';
    const newBoxID = '';
    const oldBoxID = '';
    $('blockquote.textdiff').each(function() {
        ...
        if(newBoxID != oldBoxID) {
            oldBoxID = newBoxID;
            if($(this).html() != $(this).next().html()) $(this).closest('.history-changes').before(
                diffButton);
        }
    });
});
